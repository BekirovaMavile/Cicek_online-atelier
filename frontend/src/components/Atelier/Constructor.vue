<template>
    <v-card>
        <v-tabs v-model="tab" color="blue-grey-darken-4" align-tabs="center">
            <v-tab v-for="category in categories[currentCategory]" :key="category.value" :value="category.value">
                {{ category.name }}
            </v-tab>
    ...
    <v-window v-model="tab">
          ...
          <v-window-item v-for="category in categories[currentCategory]" :key="category.value" :value="category.value">
            <v-container fluid>
              <v-row class="mb-12">
                <v-col class="d-flex child-flex" cols="12">
                  <v-row justify="center">
                    <!-- Отобразите изображения для выбранной категории -->
                    <v-img v-for="image in getCategoryImages(category.value)" :key="image" :src="image"
                      aspect-ratio="1" max-height="450"></v-img>
                  </v-row>
                </v-col>
                <v-row justify="center">
                  <v-col v-for="(checkbox, index) in getCategoryCheckboxes(category.value)" :key="index" cols="4">
                   <v-radio-group v-model="selectedCategoryValues[this.currentCategory][category.value]">
